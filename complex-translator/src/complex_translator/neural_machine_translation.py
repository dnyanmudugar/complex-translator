...
class MultilingualTokenizer:
    ...
    def fit(self, texts: list, max_vocab_size: int = 5000):
        """Wrapper method facilitating dataset interface setup."""
        self.build_vocab(texts, max_vocab_size=max_vocab_size)
...
def translate_text(text, target_tag, model, tokenizer, runtime_device, max_len=20):
    ...
    model.eval()
    src_ids = torch.tensor([tokenizer.encode(text, target_lang_tag=target_tag)]).to(runtime_device)
    ...
    with torch.no_grad():
        for _ in range(max_len):
            tgt_tensor = torch.tensor([tgt_ids]).to(runtime_device)
            output = model(src_ids, tgt_tensor)
            next_token = output[0, -1, :].argmax().item()
            tgt_ids.append(next_token)
...
class MultilingualCSVDataset(Dataset):
    def __init__(self, dataset_df, tokenizer_instance, is_train=True, all_texts=None):
        self.df = dataset_df
...
class MultilingualTransformer(nn.Module):
    def __init__(self, vocabulary_size, d_model=256, nhead=8, num_layers=4, dim_feedforward=512):
        super().__init__()
